...
import GeneralizedAdder from "../../src/components/generalized/Adder";
import { decimalToRegularVoltages } from "../../src/utils/DecimalToVoltage";
...
describe("Adder Operations - randomized testing", () => {
    let adder = new GeneralizedAdder();
    ...
    const generateRandomCases = (maxBits: number, casesPerBit: number) => {
        ...
        for (let bits = 1; bits <= maxBits; bits++) {
            for (let i = 0; i < casesPerBit; i++) {
                ...
                const aNum = Number(VoltageToBinary(a.map(v => v === DISCONNECTED ? LOW : v)));
                const bNum = Number(VoltageToBinary(b.map(v => v === DISCONNECTED ? LOW : v)));
                const cNum = Number(VoltageToBinary([c === DISCONNECTED ? LOW : c]));
                const expected = decimalToRegularVoltages(aNum + bNum + cNum, bits + 1);
    ...
    const cases = generateRandomCases(32, 100);

    test.each(cases)(`Input: [%s], [%s], %s`, (a, b, c, expected) => {
        adder.setInputs(a, b, c);
        expect(adder.getOutput()).toEqual(expected);
    });
});